...
$(function(){
	...
	proPhone.next().html("*");
	erroinfo_org=$("#errorinfo_org");
	orgCodePath=$("#orgCodePath");
	if (erroinfo_org.val()==null||erroinfo_org.val()==""){
		orgCodePath.next().html("* 上传文件大小不能超过500kb * 上传文件类型必须为：jpg、jpeg、png、pneg");
	}else{
		orgCodePath.next().html(erroinfo_org.val())
	}
	/*
	 * 验证
	 * 失焦\获焦
